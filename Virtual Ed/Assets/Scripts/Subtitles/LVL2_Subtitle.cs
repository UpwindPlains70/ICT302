    ...
    : MonoBehaviour
{
    ...
    IEnumerator Sequence()
    {
        ...
        textBox.GetComponent<TMPro.TextMeshProUGUI>().text = "This tutorial is designed to best prepare you for multiplayer mode, by building your understanding of goals for Level 2, and how you achieve these goals.";
        ...
        textBox.GetComponent<TMPro.TextMeshProUGUI>().text = "This means the maximum amount of spike proteins that you can make in this level will reduce over time, so make sure you're quick in multiplayer mode";
        yield return new WaitForSeconds(9);
        FindObjectOfType<AudioManagerNarration>().Play("#2#7");
        textBox.GetComponent<TMPro.TextMeshProUGUI>().text = "You must identify the three components required to create a spike protein";
        yield return new WaitForSeconds(5);
        FindObjectOfType<AudioManagerNarration>().Play("#2#8");
        textBox.GetComponent<TMPro.TextMeshProUGUI>().text = "You must grab some mRNA strands, tRNA molecules and some amino acids and place them into their boxes designated by matching colours.";
        yield return new WaitForSeconds(11);
        if (Android == false)
        {
